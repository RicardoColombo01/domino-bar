...
function naipeNoCanvas(c, id, x, y, r) {
  ...
  //
  // E AS TRÊS FOLHAS TÊM DE TER VÃO DE VERDADE ENTRE ELAS — relato de campo em 21/08/2026:
  // numa carta deitada na mesa (a câmera olha de cima, o naipe fica achatado na vertical) o
  // trevo virava uma mancha de "orelhas de rato". Medido: as duas folhas de baixo só se
  // TOCAVAM (a distância entre os centros era exatamente a soma dos raios — vão de largura
  // ZERO), e um vão sem largura desaparece debaixo de qualquer achatamento de câmera, não
  // só deste. Repintando a mesma receita achatada a 55% e a 40% (o que a mesa faz com uma
  // carta deitada), afastar as folhas é o que faz o vão sobreviver e o trevo continuar
  // lendo como três folhas, não uma mancha. Afastar reabre o furo do meio — um pequeno
  // triângulo cobre exatamente o ponto que a asserção do atlas sonda, mesmo lugar de antes.
  const rr = r * 0.40;
  for (const [dx, dy] of [[0, -0.50], [-0.62, 0.20], [0.62, 0.20]]) {
    c.beginPath(); c.arc(x + dx * r, y + dy * r, rr, 0, 7); c.fill();
  }
  c.beginPath();
  c.moveTo(x, y - r * 0.16); c.lineTo(x - r * 0.20, y + r * 0.14); c.lineTo(x + r * 0.20, y + r * 0.14);
  c.closePath(); c.fill();
  c.beginPath();
  c.moveTo(x - r * 0.28, y + r * 1.00);
  c.quadraticCurveTo(x, y + r * 0.42, x + r * 0.28, y + r * 1.00);
  c.closePath(); c.fill();
}
...
const texCartas = pintar('cartas', CEL_CARTA * COLS_CARTA, CEL_CARTA_A * LINS_CARTA, (c, w, h) => {
  ...
  for (let n = 0; n < LINS_CARTA; n++) {
    for (let v = 0; v < COLS_CARTA; v++) {
      ...
      //
      // O VALOR E O NAIPE TÊM DE TER VÃO ENTRE ELES — relato de campo em 21/08: "as letras
      // juntas" e "não dá pra ver a carta jogada no meio". Medido: o vão nativo entre o pé
      // do valor e o topo do naipe era de ~3px na célula (272px de altura) — sobra em uma
      // carta na mão, perto de zero numa carta jogada na mesa (que a foto mostrou rodar a
      // ~58% do tamanho da carta na mão: mais longe da câmera, sem o `ESCALA_MAO_TRUCO`
      // dedicado). Empurrar o naipe mais para baixo (0.115 → 0.16) quintuplica o vão sem
      // esbarrar no naipe grande do centro — a distância entre os dois círculos ainda sobra
      // ~8px no pior caso, contas com o raio do canto encolhido de 0.12 para 0.11 para
      // garantir a folga. O tamanho da carta jogada em si (a outra metade do relato) é a
      // hipótese mais cara da fila — mexe em `MESA_TRUCO_Z`/`ESCALA_TRUCO_MAX` (550-mesa.js)
      // e precisa de medição em várias telas antes de tocar, não neste arquivo.
      const cantoDe = (px, py, virado) => {
        ...
        naipeNoCanvas(c, naipe.id, 0, CEL_CARTA_A * 0.16, CEL_CARTA * 0.11);
        c.restore();
      };
