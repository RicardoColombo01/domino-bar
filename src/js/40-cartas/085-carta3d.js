...
const texCartas = pintar('cartas', CEL_CARTA * COLS_CARTA, CEL_CARTA_A * LINS_CARTA, (c, w, h) => {
  ...
  for (let n = 0; n < LINS_CARTA; n++) {
    for (let v = 0; v < COLS_CARTA; v++) {
      ...
      c.strokeRect(ox + CEL_CARTA * 0.06, oy + CEL_CARTA_A * 0.05,
        CEL_CARTA * 0.88, CEL_CARTA_A * 0.90);
      c.globalAlpha = 1;
      ...
      const cantoDe = (px, py, virado) => {
        ...
        // 0.23 e não 0.20: em paisagem a carta inteira tem 79px de tela, e o valor ficava com
        // 15.8px. Três pontos a mais aqui são ~3px lá, que é a diferença entre ler um "Q" e
        // adivinhá-lo.
        c.font = `bold ${Math.round(CEL_CARTA * 0.23)}px Georgia, "Times New Roman", serif`;
        c.fillText(rot, 0, -CEL_CARTA_A * 0.045);
        naipeNoCanvas(c, naipe.id, 0, CEL_CARTA_A * 0.070, CEL_CARTA * 0.090);
        c.restore();
      };
      cantoDe(0.155, 0.125, false);
      cantoDe(0.845, 0.875, true);
    }
  }
});
